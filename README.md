...
このプロジェクトには開発体験を向上させるための設定ファイルが含まれています：

#### エディタ設定

- **`.editorconfig`**: 異なるエディタ間での一貫したコーディングスタイル
- **`.vscode/settings.json`**: VS Code専用の設定（自動フォーマット、ESLint統合）
- **`.vscode/extensions.json`**: 推奨VS Code拡張機能

#### コード品質ツール

- **`prettier.config.js`**: コードフォーマット設定
- **`.nvmrc`**: Node.jsバージョン指定（nvm使用時）
- **ESLint**: TypeScript/Reactのリンティング（.eslintrcで設定）
...
npm run coverage:open   # HTMLカバレッジレポートを開く

# バンドル分析
npm run analyze         # バンドルサイズ分析（ビルド後にブラウザで開く）

# 依存関係管理
npm run deps:check      # 古い依存関係をチェック
npm run deps:update     # 依存関係を更新
npm run audit:fix       # セキュリティ脆弱性を修正

# デプロイメント
npm run deploy          # 本番環境へのデプロイ
npm run update          # システムアップデート
```
