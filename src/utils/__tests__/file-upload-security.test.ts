...
describe("FileUploadSecurity", () => {
  describe("validateSingleFile", () => {
    ...
    test("should reject file exceeding size limit", async () => {
      const file = new MockFile(["x".repeat(1000)], "large.txt", {
        type: "text/plain",
      }) as unknown as File;
    ...
    test("should reject dangerous file extensions", async () => {
      const file = new MockFile(["content"], "malware.exe", {
      ...
      expect(result.errors).toContain(
        "File type .exe is potentially dangerous and not allowed"
      );
    });

    test("should reject disallowed file extensions", async () => {
      const file = new MockFile(["content"], "script.js", {
    ...
    test("should warn about unsupported MIME types", async () => {
      ...
      expect(result.warnings).toContain(
        "MIME type application/unknown may not be fully supported"
      );
    });

    test("should sanitize unsafe filenames", async () => {
      const file = new MockFile(["content"], "../../../malicious.txt", {
        type: "text/plain",
      }) as unknown as File;
      ...
      expect(result.isValid).toBe(true);
      expect(result.warnings).toContain(
        "Filename has been sanitized for security"
      );
  ...
  describe("validateFiles", () => {
    ...
    test("should reject too many files", async () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "Too many files selected. Maximum allowed: 5"
      );
    });
    ...
    test("should reject files exceeding total size limit", async () => {
      ...
      expect(
        result.errors.some((error) =>
          error.includes("Total file size exceeds limit")
        )
      ).toBe(true);
    });
    ...
    test("should collect errors from individual files", async () => {
      const files = [
        ...
        new MockFile(["content"], "dangerous.exe", {
          type: "application/octet-stream",
        }),
      ] as unknown as File[];
      ...
      expect(result.errors.length).toBeGreaterThan(0);
      expect(
        result.errors.some((error) => error.includes("dangerous.exe"))
      ).toBe(true);
  ...
  describe("DEFAULT_UPLOAD_CONFIG", () => {
    test("should have reasonable default values", () => {
      ...
      expect(DEFAULT_UPLOAD_CONFIG.allowedMimeTypes).toContain(
        "application/json"
      );
    });
  ...
  describe("getSecurityRecommendations", () => {
    test("should return security recommendations", () => {
      ...
      expect(
        recommendations.some((rec) => rec.includes("trusted sources"))
      ).toBe(true);
      expect(recommendations.some((rec) => rec.includes("antivirus"))).toBe(
        true
      );
    });
  });
  ...
  describe("malware detection", () => {
    test("should detect potentially dangerous file signatures", async () => {
      ...
      const result = await FileUploadSecurity.validateSingleFile(
        maliciousFile,
        {
          allowedExtensions: ["txt"],
          scanForMalware: true,
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "File appears to contain suspicious content"
      );
    });
