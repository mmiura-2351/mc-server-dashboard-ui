...
export function ConnectionStatusIndicator({
  ...
  const { color, text, downtime, error, lastCheck, lastSuccessfulConnection } =
    useConnectionStatus();
  ...
  const buildTooltip = (): string => {
    ...
    // Status
    const statusKey =
      `connection.status.${text.toLowerCase().replace(/\.\.\./g, "")}` as const;
    parts.push(t(statusKey));

    // Downtime
    if (downtime) {
      parts.push(
        t("connection.indicator.tooltip.downtime", { duration: downtime })
      );
    }
    ...
    if (lastCheck && showDetails) {
      parts.push(
        t("connection.indicator.tooltip.lastCheck", {
    ...
    if (lastSuccessfulConnection && showDetails && downtime) {
      parts.push(
        t("connection.indicator.tooltip.lastSuccess", {
          time: formatTime(lastSuccessfulConnection),
        })
      );
    }
  ...
  const indicatorClasses = [styles.indicator, styles[`status-${color}`]]
    .filter(Boolean)
    .join(" ");
  ...
  return (
    ...
    >
      <div className={indicatorClasses}>
        ...
        {size !== "small" && <span className={styles.text}>{text}</span>}
      </div>

      {showDetails && downtime && (
        <div className={styles.downtime}>{downtime}</div>
      )}
    </div>
  );
}
