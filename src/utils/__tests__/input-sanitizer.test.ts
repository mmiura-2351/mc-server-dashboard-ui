...
describe("InputSanitizer", () => {
  ...
  describe("sanitizeNumeric", () => {
    test("should parse valid integer", () => {
      expect(InputSanitizer.sanitizeNumeric("123", { integer: true })).toBe(
        123
      );
    });
  ...
  describe("removeScriptTags", () => {
    ...
    test("should remove javascript and vbscript protocols", () => {
      const input =
        '<a href="javascript:alert(1)">Link</a><img src="vbscript:msgbox(1)">';
      const result = InputSanitizer.removeScriptTags(input);
    ...
    test("should remove event handlers", () => {
      const input =
        '<div onclick="alert(1)" onmouseover="evil()">Content</div>';
      const result = InputSanitizer.removeScriptTags(input);
