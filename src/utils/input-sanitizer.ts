...
export class InputSanitizer {
   ...
   */
  static sanitizeNumeric(input: string, options: { min?: number; max?: number; integer?: boolean } = {}): number | null {
   ...
   */
  static validatePassword(password: string): { isValid: boolean; errors: string[]; sanitized: string } {
    const errors: string[] = [];
   ...
   */
  static sanitizeFormData<T extends Record<string, unknown>>(data: T, rules: Partial<Record<keyof T, (value: unknown) => unknown>>): T {
